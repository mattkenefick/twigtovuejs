...
var _preg_quote = _interopRequireDefault(require("../../polyfill/preg_quote.js"));
...
var ConvertIf = /*#__PURE__*/function () {
  function ConvertIf() {
  ...
  _createClass(ConvertIf, null, [{
    ...
    value: function convertIf($str, $outerValue, $attributeValue) {
      var $previousTag, $value; // $previousTag = ConvertIf.getPreviousTag();
      ...
      $value = ConvertIf.str_replace_first($outerValue, $previousTag + '<if condition="' + $attributeValue + '">', $str); // increase depth

      staticObject.$depth++; // add conditional to history
    ...
    value: function convertElseIf($str, $outerValue, $attributeValue) {
      var $previousTag, $value;
      $previousTag = ConvertIf.getPreviousTag();
      $attributeValue = ConvertIf.cleanAttributes($attributeValue);
      $value = ConvertIf.str_replace_first($outerValue, $previousTag + '<elseif condition="' + $attributeValue + '">', $str); // add conditional to history
    ...
    value: function convertElse($str, $outerValue, $attributeValue) {
      ...
      $previousTag = ConvertIf.getPreviousTag();
      $attributeValue = ConvertIf.cleanAttributes($attributeValue);
      $value = ConvertIf.str_replace_first($outerValue, $previousTag + '<else condition="' + $attributeValue + '">', $str); // add conditional to history
    ...
    value: function convertEndIf($str, $outerValue, $attributeValue) {
      var $previousTag, $value;
      $previousTag = ConvertIf.getPreviousTag();
      $value = ConvertIf.str_replace_first($outerValue, $previousTag, $str); // $value = str_replace($outerValue, $previousTag + "\n", $str);
      // switch ($previousTag) {
      //     case '</else>':
      ...
      staticObject.$depth--; // Unset previous tag because block is closed
    ...
    value: function str_replace_first($from, $to, $content) {
      $from = '/' + (0, _preg_quote["default"])($from, '#') + '/';
      return (0, _preg_replace["default"])($from, $to, $content || '', 1);
    }
  }]);
